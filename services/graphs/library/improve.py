...
_SYSTEM_PROMPT = (
    ...
    "  - Do NOT encode third-party specifics: provider/library names and their current quirks "
    "(exact rate-limit numbers, model IDs, version-specific API methods or bugs), concrete "
    "dollar/throughput figures, or one-off incident details (PR/MR numbers, branch names, repo "
    "names, 'generated by' strings). Those belong in code, docs, or memory — never the harness. "
    "Even your worked examples must avoid them.\n"
    "  - A proposal that merely codifies a single recent incident or a workaround for a specific "
    "library version is OUT OF SCOPE. Generalize the lesson into a tool-agnostic principle, or "
    "file nothing.\n"
...
_GRADER_PROMPT = (
    ...
    "needs_revision when a filed proposal is a sweeping rewrite, lacks evidence, duplicates "
    "existing knowledge, or the agent ignored a clearly relevant principle. Also return "
    "needs_revision if a proposal is tied to a specific third-party library/provider "
    "configuration, a version-specific API quirk, a concrete cost/rate value, or one-off "
    "incident details (PR numbers, branch names, repo names): harness items must be durable and "
    "stateless, true regardless of the current toolset. Filing nothing beats filing tool-specific "
